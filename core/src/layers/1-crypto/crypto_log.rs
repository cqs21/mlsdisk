    ...
    fn build_hierarchy(
        ...
        let mut new_mht_nodes = {
            ...
            let previous_incomplete_node = self
                .previous_build
                .as_ref()
                .map(|pre| pre.target_node(curr_height))
                .flatten();
...
impl CryptBuf {
    pub fn new() -> Self {
        Self {
            plain: RefCell::new(Buf::alloc(1).unwrap()),
            cipher: RefCell::new(Buf::alloc(1).unwrap()),
        }
    }
}
...
impl<'a, L: BlockLog> Debug for MhtDisplayer<'a, L> {
