...
use crate::layers::lsm::{
    AsKV, LsmLevel, MasterSyncId, RangeQueryCtx, RecordKey as RecordK, RecordValue as RecordV,
...
impl<D: BlockSet + 'static> SwornDisk<D> {
    ...
    pub fn create(disk: D, root_key: Key) -> Result<Self> {
        ...
        let logical_block_table = {
            ...
            let master_sync_id = MasterSyncId::new(Self::subdisk_for_master_sync_id(&disk)?);
            TxLsmTree::format(
                tx_log_store.clone(),
                listener_factory,
    ...
    pub fn open(disk: D, root_key: Key) -> Result<Self> {
        ...
        let logical_block_table = {
            ...
            let master_sync_id = MasterSyncId::load(Self::subdisk_for_master_sync_id(&disk)?)?;
            TxLsmTree::recover(
                tx_log_store.clone(),
                listener_factory,
                Some(Arc::new(on_drop_record_in_memtable)),
    ...
    fn subdisk_for_logical_block_table(disk: &D) -> Result<D> {
        disk.subset(disk.nblocks() * 15 / 16..disk.nblocks() - 1) // TBD
    }

    fn subdisk_for_master_sync_id(disk: &D) -> Result<D> {
        // Use last block for storing master sync ID for now
        disk.subset(disk.nblocks() - 1..disk.nblocks()) // TBD
    }
}
